...
//=================================
function submitForm() {
    var sbrqq = document.uiForm.sbrqq.value;
    var sbrqz = document.uiForm.sbrqz.value;
    var cbrqq = document.uiForm.cbrqq.value;
    var cbrqz = document.uiForm.cbrqz.value;

    //申报日期不为空判断
    if (sbrqq == null || sbrqq == "" || sbrqz == null || sbrqz == "") {
        Ext.Msg.alert("错误", "请选择申报日期!");
        return;
    }
    if (cbrqq == null || cbrqq == "" || cbrqz == null || cbrqz == "") {
        Ext.Msg.alert("错误", "请选择抄报日期!");
        return;
    }
    if (cbrqq < sbrqq || cbrqz > sbrqz) {
        Ext.Msg.alert("错误", "抄报日期选择超出范围,请您核对后重新选择!");
        return;
    }
