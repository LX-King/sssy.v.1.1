// 提交表单
function submitForm() {
    document.taxInvoiceSalesForm.submit();
}

// 重置表单
function resetForm() {
    document.taxInvoiceSalesForm.reset();
}

/* 设置年份select */
function setYearSelect(selectId) {
    var currentYear = new Date().getFullYear();
    for (var i = 2009; i <= 2020; i++) {
        $("#" + selectId).append("<option value=" + i + ">" + i + "</option>");
        $("#" + selectId).val(currentYear);		// 设置选中当前年
    }
}

function setMonthSelect(id) {
    $("#" + id).append("<option value='1'>1月</option>");
    $("#" + id).append("<option value='2'>2月</option>");
    $("#" + id).append("<option value='3'>3月</option>");
    $("#" + id).append("<option value='4'>4月</option>");
    $("#" + id).append("<option value='5'>5月</option>");
    $("#" + id).append("<option value='6'>6月</option>");
    $("#" + id).append("<option value='7'>7月</option>");
    $("#" + id).append("<option value='8'>8月</option>");
    $("#" + id).append("<option value='9'>9月</option>");
    $("#" + id).append("<option value='10'>10月</option>");
    $("#" + id).append("<option value='11'>11月</option>");
    $("#" + id).append("<option value='12'>12月</option>");
...
function registerOnSelectChange() {
    ...
    $("form[name='taxInvoiceSalesForm']").delegate("select", ajaxEvent, function () {
        var curSelect = $(this);
        $.ajax({
            url:url,
...
$(document).ready(function () {
    /*初始化年月日*/
    setYearSelect('nd');
    setMonthSelect('monthPeriod');
    setDaySelect('dayPeriod');
