...
import com.tyut.sssy.task.service.MacroTaskTestService;

/**
 * 
 * 项目名称：sssy20120713
 * 类名称：MacroTaskTestAction  
 * 类描述：总/分量任务考核  
 * 创建人：梁斌  
 * 创建时间：2012-7-21 上午10:12:55  
 * 修改人：梁斌  
 * 修改时间：2012-7-21 上午10:12:55  
 * 修改备注：  
 * @version 
 *
 */
public class MacroTaskTestAction {

	/**
	 * 显示总/分量任务考核操作选择界面
	 * @return
	 */
	public String showMacroTaskTestUI() {
		...
		return "show_macro_task_test_ui";
	}
	...
	public String showRwzxryList() {
		...
		//  根据反馈日期时间起止获得执行人员列表
		MacroTaskTestService macroTaskTestService = new MacroTaskTestService();
		List<User> userList = macroTaskTestService.getRwzxryListByRwfkRq(sjQ, sjZ);
	...
	public String showSubmitTestReport() {
		...
		String rwztDm = "05";
		List<MacroTask> macroTaskList = macroTaskService.getListByRwfkRqAndRwztAndRwzxryDm(sjQ, sjZ, rwztDm, rwzxryDm);
		
		for (MacroTask macroTask : macroTaskList) {
			rwjsDf += macroTask.getRwjsDf();
			rwfkDf += macroTask.getRwfkDf();
			rwzxDf += macroTask.getRwzxDf();
		}
		
		int num = macroTaskList.size();
		rwjsDf = rwjsDf / num;
		rwfkDf = rwfkDf / num;
		rwzxDf = rwzxDf / num;
	...
	public String submitTestReport() {
		...
		// 根据被考核人员和起止时间删除原有记录
		MacroTaskTestService macroTaskTestService = new MacroTaskTestService();
		macroTaskTestService.deleteByBkhryAndSjQAndSjZ(bkhryDm, sjQ, sjZ);
		...
		macroTaskTestService.add(taskTest);
	...
	private String getNewRwkhDm() {
		...
		MacroTaskTestService macroTaskTestService = new MacroTaskTestService();
		String maxRwkhDm = macroTaskTestService.getMaxRwkhDm(dateStr);
		String rwkhDm = "";
	 ...
	 * @return
	 */
	public String showMacroTaskTestOrderTable() {
		...
		MacroTaskTestService macroTaskTestService = new MacroTaskTestService();
		String khrwLx = "hgrw";
		List<TaskTest> taskTestList = macroTaskTestService.getTaskTestListByKhRq(sjQ, sjZ, khrwLx);
		
		request.setAttribute("taskTestList", taskTestList);
		request.setAttribute("rwkhRq", rwkhRq);
		
		return "show_macro_task_test_order_table";
	}
