...
public class RegisterTypeGraphicAction extends ActionSupport implements Preparable {

    RegisterTypeGraphicDao dao = null;
    FirmRegTypeDao pDao = null;

    public void prepare() throws Exception {
        pDao = new FirmRegTypeDao();
        dao = new RegisterTypeGraphicDao();
    }
    ...
    private void query() {
        List<RegisterTypeGraphic> list = this.dao.queryAll();
    ...
    private String showModify(HttpServletRequest request) {
        ...
        List<FirmRegType> list = pDao.getFirmRegTypeList();
        RegisterTypeGraphic obj = this.dao.queryById(id);
    ...
    private void modify(HttpServletRequest request) {
        ...
        registerTypeGraphic.setId(id);
        registerTypeGraphic.setQyzclxDm(code);
        registerTypeGraphic.setMc(name);
        boolean flag = this.dao.modify(registerTypeGraphic);
        if (flag)
            printJson("{\"msg\":\"succ\"}");
        else
    ...
    private void del(HttpServletRequest request) {
        int id = Integer.parseInt(request.getParameter("id"));
        RegisterTypeGraphic firmRegType = this.dao.queryById(id);
