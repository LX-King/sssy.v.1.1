 ...
 */
public class TaxFinishGraphicAction extends ActionSupport implements Preparable {

	private TaxCollectionSearchService service = null;

	private TaxCollectionFinishTableParameter parameter = null;
	...
	public void prepare() throws Exception {
		...
		this.parameter = (TaxCollectionFinishTableParameter) session
				.getAttribute(SessionAttributeKey.SSSRWC);
		this.service = new TaxCollectionSearchService();
		this.dataset = new LinkedHashMap<String, List<BigDecimal>>();
		this.categories = new ArrayList<String>();
        if(!this.parameter.getPd().equals("月")){
            this.parameter.setPd("月");
        }
	...
	private void drawLineGraphic() {
		...
		//报表期
		String reportPeriod = parameter.getYf().trim().equals("1") ? parameter
				.getNd()
				+ "年" + "1月" : parameter.getNd() + "年" + " 1 - "
				+ (Integer.parseInt(parameter.getYf())) + " 月";
		//税务机关单位名称
		...
		FushionChartLineJsonData dataSource = new FushionChartLineJsonData(
				caption, xAxisName, yAxisName, this.categories, this.dataset);
	...
	private void drawPieGraphic() {
		...
		for (Map.Entry<String, List<BigDecimal>> entry : this.dataset
				.entrySet()) {
			if (entry.getKey().trim().equals("合计")) {
				...
				dataSet.put(entry.getKey(), tempOther.divide(total, 5,
						BigDecimal.ROUND_HALF_EVEN));
			}
		}
		String displayItem = this.parameter.getFl();
		if (displayItem.equals("02") || displayItem.equals("05")
				|| displayItem.equals("03"))
			dataSet.put("其它", other
					.divide(total, 5, BigDecimal.ROUND_HALF_EVEN));
		FushionChartPieJsonData dataSource = new FushionChartPieJsonData(
				caption, xAxisName, yAxisName, dataSet);
		//        FushionChartDataSource dataSource = generateLineDataSource(caption, xAxisName, yAxisName, displayItemService.getById(displayItem).getFlmc());
		printJson(dataSource.toJson());
	...
	private void drawColumnGraphic() {
		...
		String xAxisName = "本期";
		String yAxisName = "完成税收";
		FushionChartColumnJsonData dataSource = new FushionChartColumnJsonData(
				caption, xAxisName, yAxisName, this.categories, this.dataset);
		printJson(dataSource.toJson());
	}
	...
	private void extractData(int month) {
		...
		if (displayItem.equals("02")) {/*行业*/
			...
			for (int i = 1; i <= month; i++) {
				...
				List<TaxCollectionFinishTable> finishTable = service
						.getFinishTable(parameter);
				totalList.add(finishTable.get(0).getA2());
				...
				for (int j = 0; j < list.size(); j++) {
					...
					for (TaxCollectionFinishTable table : finishTable) {
						if (currIndustry.getIndustryName().trim().equals(
								table.getXm().trim())) {
							if (table.getA2() != null)
								tempList.add(table.getA2());
							else
			...
			for (int i = 1; i <= month; i++) {
				...
				parameter.setYf(temp);
				List<TaxCollectionFinishTable> finishTable = service.getFinishTable(parameter);
				totalList.add(finishTable.get(0).getA1());
			...
			for (int i = 1; i <= month; i++) {
				...
				parameter.setYf(temp);
				tempList.add(service.getFinishTable(parameter).get(0).getA2());
			}
		}
	}
	 ...
	 */
	private FushionChartDataSource generateLineDataSource(String caption,
			String xAxisName, String yAxisName, String displayItem) {
		...
		for (Map.Entry<String, List<BigDecimal>> entrySet : this.dataset
				.entrySet()) {
			FushionChartDataSet dataSet1 = new FushionChartDataSet(entrySet
					.getKey());
		...
		FushionChartDataSource dataSource = new FushionChartDataSource(chart,
				categoriesList, dataSet);
		return dataSource;
	}
	 ...
	 * @return FushionChartDataSource
	 */
	private FushionChartDataSource generatePieDataSource(String caption,
			String xAxisName, String yAxisName, String displayItem) {
		FushionChartDesc chart = new FushionChartDesc();
		chart.setCaption(caption);
		chart.setXAxisName(xAxisName);
