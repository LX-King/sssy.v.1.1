 ...
 */
public class TaxOwnForDetailGraphicAction extends ActionSupport implements Preparable{

    private TaxCollectionSearchService service = null;

    private OwnPayTaxDetailTableParameter parameter = null;
    ...
    public void prepare() throws Exception {
        ...
        this.parameter = (OwnPayTaxDetailTableParameter) session.getAttribute(SessionAttributeKey.QJSJMX);
        this.parameter.setFl("sz");
...
//        FushionChartDataSource dataSource = generateLineDataSource(caption, xAxisName, yAxisName, displayItemService.getById(displayItem).getFlmc());
    ...
    private String generateCaption() {
        ...
        //图表的标题
        return unitName + reportPeriod + "欠缴税金明细情况图表";
    }
    ...
    private void extractData(int month) {
        ...
        for (int i = 1; i <= month; i++) {
            ...
            List<OwnPayTaxDetailTable> finishTable = service.getOwnPayTaxDetailTable(parameter);
            for (OwnPayTaxDetailTable t : finishTable) {
                if (t.getXm().trim().equals("合计")) {
                    totalList.add(t.getA4()==null?new BigDecimal(0):t.getA4());
                } else {
                    if (this.dataset.containsKey(t.getXm().trim())) {
                        this.dataset.get(t.getXm().trim()).add(t.getA4()==null?new BigDecimal(0):t.getA4());
                    }
                }
            }
            for (Map.Entry<String, List<BigDecimal>> entry : this.dataset.entrySet()) {
