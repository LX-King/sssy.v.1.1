 ...
 */
public class TaxResourceSumChangeGraphicAction extends ActionSupport implements Preparable {

    private TaxResourceSearchService service = null;

    private TaxResourceSumChangeTableParameter parameter = null;
    ...
    public void prepare() throws Exception {
        ...
        this.parameter = (TaxResourceSumChangeTableParameter) session.getAttribute(SessionAttributeKey.SYZLZJ);
    ...
    private void extractData(int month) {
        ...
        List<String> list = Arrays.asList(Constants.TAX_RESOURCE_SUM_CHANGE);
        ...
        for (int i = 1; i <= month/3; i++) {
            ...
            List<TaxResourceSumChangeTable> finishTable = service.getTaxResourceSumChangeTable(parameter);
            for (TaxResourceSumChangeTable t : finishTable) {
                if (t.getA().trim().equals("合计")) {
                   for(Map.Entry<String,List<BigDecimal>> entry :this.dataset.entrySet()){
                       if(entry.getKey().trim().equals("营业收入")){
                           entry.getValue().add(t.getB2());
                       }else if(entry.getKey().trim().equals("利润总额")){
                           entry.getValue().add(t.getC2());
                       }else if(entry.getKey().trim().equals("产品产量")){
                           entry.getValue().add(t.getD2());
                       }else if(entry.getKey().trim().equals("产品销量")){
                           entry.getValue().add(t.getE2());
                       }else{
                           entry.getValue().add(t.getF2());
                       }
                   }
                    break;
                }
