...
public abstract class BaseDaoJdbcImpl<T> {
    ...
    public String queryMaxId(String sql) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String maxId = null;
     ...
     * 分页查询
     *
     * @param sqlPagation 分页查询
     * @return PageModel
     */
    public List<T> queryByCon(String sqlPagation) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<T> list = new ArrayList<T>();
        List<T> pageModel = null;

        try {
            conn = C3P0Util.getConnection();
            ps = conn.prepareStatement(sqlPagation);
            rs = ps.executeQuery();
            while (rs.next()) {
                T t = objectRelationMapping(rs);
                list.add(t);
            }
